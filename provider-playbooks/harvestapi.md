...
**Best for:** LinkedIn data without cookies or an account: keyword post search, company and profile posts, reactions and comments on a post, full profiles, employee search. It is the provider behind Deepline's LinkedIn plays and the raw material of "social buying signals" (what Sillage packages).
...
**Pricing basis:** per_result. Public Apify prices (2026-09): ~$2 per 1k posts, reactions or comments; ~$4 per 1k full profiles; profile search $0.10 per page + $0.004 per profile. The direct API (api.harvestapi.io) is a subscription whose limit is **concurrency**, not calls per minute (Deepline runs on a Business plan with 40 concurrent requests). Endpoint paths and header in the adapter are marked `verify against docs`.

**Pitfalls (from Deepline's playbook and ours):**
