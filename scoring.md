...
Evidence first (16 method references in `references/scoring/`); rules or a separately evaluated model; never weights derived from phrase counts.
...
- `src/core/scoring-contract.ts` — the pure replay scorer (a strict point-in-time contract): point-in-time checks (`known_at`, `event_at`, `retrieved_at` all before the scoring instant), source-class rules (fit = external, engagement = first-party events, AE facts excluded), staleness windows, midrank percentile grading.
