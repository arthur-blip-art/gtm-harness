...
Use this reference when comparing `/deepline-research` to `last30days` or when designing the actual the harness workflow.
...
- the harness provider catalog is not part of Stage 1. Search and describe the harness tools only after the public-source synthesis identifies which source families, private joins, probes, costs, and gaps matter.
...
| `tool_or_provider` | the harness tool id, provider, connector, or proposed gap |
...
| `cost_basis` | the harness credit basis or unknown until describe/probe |
| `confidence` | extraction/match confidence |
| `provenance` | query, supplemental lead, run id, file id, or connector object path |

### Stage 4: Score

Preserve `last30days`-style relevance, recency, engagement, and source-quality scoring. Add the harness-specific score inputs:
...
- cost/coverage value: does the source justify its the harness credit cost?
...
Then add the harness-specific consolidation:
...
4. "the harness route" - only now map sources to the harness tools, costs, probes, and gaps.

For example, in healthcare provider analysis, "NPI registry has no native the harness tool" is not the conclusion. The conclusion is: "NPI taxonomy is a high-recall seed but over-includes non-ICP storefronts; pull it via generic web/API route, then validate against Maps/website identity and join proprietary win/loss or CRM outcomes."

## Better-Than-last30days Criteria

Claim the harness is better only when the workflow:
...
- quotes the harness-facing cost before scaled execution
